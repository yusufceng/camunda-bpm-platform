...
COPY engine-rest/assembly/target/camunda-engine-rest-*.war /tmp/
RUN TOMCAT_DIR=$(find /opt/camunda -name "apache-tomcat-*" -type d) && \
    cp /tmp/camunda-webapp*.war ${TOMCAT_DIR}/webapps/camunda.war && \
    cp /tmp/camunda-engine-rest-*.war ${TOMCAT_DIR}/webapps/engine-rest.war && \
    rm /tmp/camunda-webapp*.war /tmp/camunda-engine-rest-*.war && \
    echo "=== WAR Files Deployed ===" && \
    ls -la ${TOMCAT_DIR}/webapps/
