...
# Copy built artifacts (Maven package phase generates these)
# Note: These files must exist in build context, Maven package should create them

# Engine core libraries
COPY engine/target/*.jar /camunda/lib/

# DMN engine libraries  
COPY engine-dmn/engine/target/*.jar /camunda/lib/
COPY engine-dmn/feel-api/target/*.jar /camunda/lib/
COPY engine-dmn/feel-juel/target/*.jar /camunda/lib/
COPY engine-dmn/feel-scala/target/*.jar /camunda/lib/
